...
// TEMPORARILY DISABLED - Using simplified auth instead
// import { createClient } from '@supabase/supabase-js';
import Constants from 'expo-constants';
// Import the shared Supabase client to ensure session consistency
import { supabase as sharedSupabaseClient } from './supabase';

// Initialize the WebBrowser module
WebBrowser.maybeCompleteAuthSession();

// Environment configuration - fixed for mobile detection
const IS_PRODUCTION = process.env.APP_ENV === 'production' || 
  (typeof window !== 'undefined' && (
    window.location.hostname.includes('expo.app') || 
    window.location.hostname.includes('expo.dev') ||
    (!window.location.hostname.includes('localhost') && !window.location.hostname.includes('192.168'))
  ));
...
// Use the shared client to ensure session consistency across the app
let supabaseClient = sharedSupabaseClient;

if (!supabaseClient) {
  // Create a client with the same configuration as the shared one
